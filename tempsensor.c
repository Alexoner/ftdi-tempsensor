...
int buf2i(unsigned char *buffer,int *res,size_t n)
{
    //buffer to integer
    int i;
...
int buf2f(unsigned char *buffer,float *res)
{
...
FT_HANDLE scan_device()
{
    ...
    for(i=0; ((i<MAX_DEVICES)&& (i<iNumDevs)); i++)
    {
		...
		if(verify_tempsensor(fthandle[i]))
		{
			//FT_Close(fthandle[i]);
			return fthandle[i];
		}
...
{//transmit instructions to the device and return the length of received data
	...
	if(ftstatus!=FT_OK)
	{
		perror("handle_device()->FT_GetQueueStatus()");
		return -1;
	}
	ftstatus=FT_Read(fthandle,rxbuf,bytes,rxBytes);
	if(ftstatus!=FT_OK)
	{
		perror("handle_device()->FT_Read()");
		return -1;
	}
	else
	{
...
int verify_tempsensor(FT_HANDLE fthandle)
{
	...
	rxbytes=handle_device(fthandle,txbuf,rxbuf,txbytes,&rxbytes);
	if(rxbytes<=0)
	{
...
int query_temperature(FT_HANDLE fthandle,float *t)
{
    ...
    else
    {
		...
		if(memcmp(rxbuf,ERROR_TEMP,sizeof(ERROR_TEMP))==0)
		{
			return -1;
		}
		buf2f(rxbuf,t);
...
int save2json(long ti,float temp)
...
int get_extrema_temp(char *path,time_t time_start,time_t time_stop,
		float *max_temp,float *min_temp,float *ave_temp)
{
	char buf[MAX_BUF];
	time_t ti;//time read from file
	float temp,total_temp=0;
	int n=0,i=0,flag=0;//flag indicates whether max and min are initialized
		...
		else
		{
			...
			if(flag==0)
			{
				...
				*min_temp=temp;
				flag=1;
				i++;
				total_temp+=temp;//total temperature stacks up
			}
			else
